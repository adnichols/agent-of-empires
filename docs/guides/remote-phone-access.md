...
- **Two factors at first pairing**: the auth token in the QR URL, plus the passphrase typed on the login page. Either alone is useless. After pairing, the device is bound and stays signed in while the token remains stable.
...
- **The token does not expire on a timer.** The QR URL token stays stable so a sleeping phone or installed PWA can reopen without catching a short refresh window. You won't see the QR / token-paste prompt again unless site data is cleared, the session expires, devices are signed out, or the token file is replaced.
...
- **401 or "missing auth token"**: scan the QR, not a screenshot of the URL without the `?token=...` query. If you previously paired before this change and the phone is stuck on an expired token, scan the current QR once to save the stable token.
